...
class TN():
    ...
    def _logderivative(self, x):
        ...
        derivative : numpy array, shape (m_parameters,)
        """
        # ~ print(self._probability(x))
        # ~ print("++++++++")
        # ~ print(len(self._derivative(x)))
        # ~ print("===========")
    ...
    def _fit(self, v):
        ...
        """
        tmp = self._likelihood_derivative(v)
        tmp[tmp > 10] = 10
        tmp[tmp < -10] = -10
        # ~ update_w = self._likelihood_derivative(v)
        self.w -= self.learning_rate * tmp#update_w
    ...
    def _weightinitialization2(self,rng):
        ...
        rng : random number generator
        """
        self.m_parameters2=(self.n_features)*self.d*self.D*self.D
        # ~ print(self.m_parameters2)
        # ~ print("***")
    ...
    def fit(self, X, w_init=None, epsilon = 0.0):
...
#       Initialize parameters of MPS
        ...
        if w_init is None:
            ...
            self.w = self._unpadding_function(w_init)
            # ~ print(self.w[0:10])
            self.w = self._epsilon_padding(self.w, epsilon)
        
        self.m_parameters = len(self.w)
        self.norm=self._computenorm()
        self.history=[]
        ...
        # ~ x = self._unpadding_function(self.w)
        # ~ print(x[0:10])
        return self

    def fit_tensor(self, X, w_init=None, epsilon = 0.0):
...
#       Initialize parameters of MPS
        ...
        if w_init is None:
            ...
            self.w = self._unpadding_function(w_init) #self.w=w_init
            self.w = self._epsilon_padding(self.w, epsilon)
        self.norm=self._computenorm()
        self.history=[]
        # ~ print(self.m_parameters)
        ...
        # ~ print(res)
        # ~ self.w = res.x.view(self.w.dtype)
        self.norm=self._computenorm()
        # ~ print(self.w[0:10])
        # ~ print("--**--")
        # ~ print(res.x.view(self.w.dtype)[0:10])
        end = time.time()
        print("KL divergence = %.6f, time = %.2fs" % (self.distance(X),end - begin))
        return self
		
    # ~ def fit_tensor(self, X, w_init=None, init_val = None):
        # ~ """Fit the model to the tensor X, with parameters initialized at w_init
        # ~ Parameters
        # ~ ----------
        # ~ X : {numpy array, non-negative tensor} shape (d, d, d, d,...) (dimension d^n_features)
...
# ~ #       Initialize parameters of MPS
        ...
        # ~ print(initial_value[0:10])
        # ~ print(len(initial_value))
        # ~ print(initial_value.dtype)
        # ~ res=minimize(fun=distancepartial,jac=derivativedistancepartial,x0=initial_value.view(np.float64),\
                     # ~ method='L-BFGS-B',options={'maxiter': self.n_iter},tol=10**(-16))
